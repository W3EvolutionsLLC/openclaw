...
export function createCrablineProviderDelivery(
  ...
  const providerDelivery = adapter.createAgentDelivery({
    target:
      adapter.channel === "telegram"
  ...
  const delivery = {
    channel: providerDelivery.channel,
    replyChannel: providerDelivery.replyChannel,
    replyTo: providerDelivery.replyTo,
    to: providerDelivery.to,
  };
  return {
    delivery,
    providerTargetKey:
