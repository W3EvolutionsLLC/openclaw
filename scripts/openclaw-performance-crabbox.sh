...
clone_exact() {
  ...
  [[ "$(git -C "$destination" rev-parse HEAD)" == "$sha" ]] || die "${repository} checkout drifted"
  [[ "$(git -C "$destination" remote get-url origin)" == "https://github.com/${repository}.git" ]] ||
...
prepare_sut() {
  ...
  dirty_env="$(
    as_sut env |
      grep -E '^(ACTIONS_|AWS_|CRABBOX_|GITHUB_|RUNNER_)' || true
  )"
...
EOF
  ...
  local report
  report="$(find "$report_dir" -maxdepth 1 -type f -name '*.json' ! -name plan.json -print -quit)"
...
remote_main() {
  ...
  if ((EUID != 0)); then
    ...
    exec sudo /usr/bin/env -i PATH=/usr/sbin:/usr/bin:/sbin:/bin /bin/bash -c \
      'install -D -o root -g root -m 0755 "$1" "$2"; shift 2; exec "$0" "$@"' \
      "$root_script" "$0" "$root_script" remote "$@"
  fi
  ...
  set -e
  quiesce_sut
  [[ "$(git -C "$root/openclaw" rev-parse HEAD)" == "$openclaw_sha" ]] ||
    die "OpenClaw HEAD changed during SUT execution"
  [[ "$(git -C "$root/kova" rev-parse HEAD)" == "$kova_sha" ]] ||
...
verify_payload() {
  (($# == 5)) || die "verify mode requires lane, timing, evidence, payload, and output"
  local lane="$1" timing="$2" evidence="$3" payload="$4" output="$5"
  local tmp
  tmp="$(mktemp -d)"
  VERIFY_TMP="$tmp"
  ...
  jq -e '
    (.leaseId | type == "string" and length > 0) and
    .leaseStopped == true and ((.leaseStopError // "") == "")
  ' "$timing" >/dev/null ||
    die "Crabbox did not prove lease cleanup"
  jq -e --arg lane "$lane" \
  ...
  jq --slurpfile timing "$timing" \
    '.lease += {
      id:$timing[0].leaseId,
      stopped:$timing[0].leaseStopped,
      stopError:($timing[0].leaseStopError // "")
    }' "$evidence" > "$output"
}
