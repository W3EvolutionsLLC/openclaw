...
describe("SystemAgentChatEngine", () => {
  ...
  it("rejects a QR poll that resumes after its engine is disposed", async () => {
    let settleOwner!: () => void;
    const owner = new Promise<void>((resolve) => {
      settleOwner = resolve;
    });
    ...
    const poll = engine.pollStep(stepId);
    await Promise.resolve();
    const disposal = engine.dispose();

    await expect(poll).rejects.toThrow("System-agent chat engine has been disposed");
