...
export class PluginInstallCommandFailure extends Error {
  constructor(
    message: string,
    readonly code?: string,
  ) {
    super(message);
    this.name = "PluginInstallCommandFailure";
  }
}
...
async function runPluginInstallCommandUnlocked(
  ...
  const explicitInstallPolicyAcknowledgement = opts.onInstallPolicyWarning
    ...
    : resolveInstallPolicyAcknowledgementCliOptions({
        ...
        action: "install",
        reportError: (message) => runtime.error(message),
      });
      ...
      const installFromClawHub = async (
        ...
        if (!result.ok) {
          if (params.clawManaged) {
            throw new PluginInstallCommandFailure(result.error, result.code);
          }
          if (!isClawHubBlockedCliFailure(result)) {
            runtime.error(result.error);
          }
