...
function runtimeBinding(role: "baseline" | "candidate") {
  ...
  return {
    installedRuntime: {
      ...
      packageSha256: candidate ? "f".repeat(64) : GATEWAY_NODE_COMPAT_BASELINE_SHA256,
      sourceSha,
      version,
    },
    packagedArtifact: {
      ...
      sha256: candidate ? "f".repeat(64) : GATEWAY_NODE_COMPAT_BASELINE_SHA256,
      sourceSha,
      version,
    },
  };
...
describe("Gateway/node compatibility release evidence", () => {
  ...
  it("rejects baseline bindings that do not match the canonical source or tarball", () => {
    ...
    wrongSource.set(
      sourcePath,
      Buffer.from(canonicalizeGatewayNodeCompatEvidence(sourceEvidence), "utf8"),
    );
    expect(() =>
    ...
    wrongDigest.set(
      sourcePath,
      Buffer.from(canonicalizeGatewayNodeCompatEvidence(digestEvidence), "utf8"),
    );
    expect(() =>
